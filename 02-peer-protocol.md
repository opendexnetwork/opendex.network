...
This section describes how to set up a connection between two nodes for performing updates, trades, and swaps. 

Each node maintains a persistent **secp256k1** private key that uniquely identifies the node. We recommend only allowing manual resets of the private key, for example by deleting a file or database entry. 
...
The initiator must know the recipient's identity in advance. The recipient learns the initiator's identity by receiving the `SessionAck` message.

By the end of the handshake, two distinct shared keys are created, one for each side of the communication, to be used to encrypt all messages during the session lifetime. 
...
If the fields of the `SessionInit` message are valid, the receiver replies with a `SessionAck` message. If a `SessionAck` message is not received within a reasonable time frame (usually 10 seconds), the sender may disconnect.
...
These messages are used to maintain the P2P overlay once a session has been established via the initial handshake.
...
The sender of the `Ping` message may disconnect if it does not receive a `Pong` message back within 10 seconds. 
...
The `Pong` message sent in response to the `Ping` message. It serves to keep the connection alive by explicitly notifying the other end that the receiver is still active. 
...
The `Disconnecting` message is used to inform a connected peer that a disconnection is imminent; If received, the peer should disconnect immediately. A well-behaved host that sends a `Disconnecting` message allows the peer at least two seconds to disconnect before disconnecting himself.

`reason` is an optional parameter to specify one of the following reasons for the disconnection:
